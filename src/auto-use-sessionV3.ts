...
const hanleEn = async ({ page, result }: { page: Page; result: number }) => {
  ...
  try {
    ...
    const count = await verify.count();
    console.log(`🔍 Tìm thấy ${count} phần tử chính xác có text 'Remove'`);
    if (count >= 0) {
      await page.waitForTimeout(1000 + randomDelay());
      const element = verify.nth(1);
      await element.hover();
      await element.click({ delay: randomDelay(150, 300) });
      console.log('✅ Đã click vào phần tử Remove');
    } else {
      console.log('⚠️ Không tìm thấy phần tử Remove');
    }
  } catch (error: any) {
    console.log('❌ Lỗi khi click vào Remove:', error.message);
  }
  await page.waitForTimeout(15000);
  const successText = page.locator('span', {
    hasText: /^Account spending limit has been removed$/,
  });
  if (await successText.isVisible({})) {
    console.log('✅ Có Thành công Account spending limit has been removed');
    result = 1;
  } else {
    console.log(
      '❌ Không có Thành công Account spending limit has been removed',
    );
  }
  return result;
};
...
// phần tiếng việt =====================================================
const hanleVi = async ({ page, result }: { page: Page; result: number }) => {
  await page.waitForLoadState('networkidle');
  let isVerify = 0;
  try {
    const verify = page.locator('div', {
